...
async def publish(bot,post,api):
    ...
    await api("PUT","/internal/discussions/articles",{"ghost_post_id":post_id,"forum_id":str(forum.id),
        "thread_id":str(thread.id),"thread_url":thread.jump_url,"title":post["title"][:300],
...
async def sync_published(bot,api):
    ...
    result=await publisher.request("GET","posts/?limit=25&status=published&include=authors,tags")
